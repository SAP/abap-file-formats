INTERFACE zif_aff_tran_v1
  ...
  TYPES:
    BEGIN OF ty_dia,
      ...
      "! If this indicator is set, activating a transaction variant as standard transaction variant is enabled in the transaction variant maintenance.
  ...
  TYPES:
    BEGIN OF ty_oo,
      ...
      "! If this indicator is set, the OO transaction uses a local ABAP class within this specified ABAP program. Otherwise it uses a global ABAP class.
      ...
      "! <p class="shorttext">Update Mode</p>
      "! The update mode for an OO transaction defines how data changes are processed: synchronously, asynchronously, or immediately.
  ...
  TYPES:
    ...
    END OF ty_par_pv.
  TYPES ty_par_pv_t TYPE STANDARD TABLE OF ty_par_pv WITH EMPTY KEY.

  TYPES:
    BEGIN OF ty_par,
      "! <p class="shorttext">Parent Transaction</p>
      "! The parent transaction code of the parameter transaction defines the transaction that provides the program and the dynpro number
      "! that are started when the transaction is called. The parent transaction can be a dialog or a report transaction.
      "! $required
      par_parent_transaction_code TYPE c LENGTH 20,
      "! <p class="shorttext">Skip Initial Screen</p>
      "! Skip initial screen mode allows bypassing the first screen of the parent transaction by using predefined values.
  ...
  TYPES:
    BEGIN OF ty_var,
      ...
      "! It enables inheritance of settings like UI attributes or authorizations, ensuring consistency and reducing duplication in configuration.
      "! $required
      var_parent_transaction_code TYPE c LENGTH 20,
      "! <p class="shorttext">Cross Client</p>
      "! If the indicator is set, the transaction variant is cross-client. If the switch is not set, the transaction variant is client-specific.
  ...
  TYPES:
    ...
    "! whether an authorization default is added to a role’s data by the profile generator when the app is in the role menu.
  ...
  CONSTANTS:
    ...
    BEGIN OF co_maintenance_mode,
      ...
      "! New objects whose check has been detected by the authorization trace are added to the object list in change mode with the maintenance status Not maintained.
      ...
      "! Only authorization objects from the application areas Basis (S_*) and HR (P_* and PLOG) are automatically added with the default status No Default.
      ...
      "! This mode is used to classify applications that are not being developed any more and because of this do not meet important functional requirements or
    ...
    BEGIN OF co_maintenance_status,
      ...
      "! This status is given to all objects that are not required for the execution of functions in the application.
      "! The authorization check is executed in exactly the same way the statuses described above.
      no_default             TYPE ty_maintenance_status VALUE 'X',
      "! <p class="shorttext">Default With Field Values</p>
      "! This default status is assigned to all objects that are used to protect functions within the associated application and
      "! for which the delivery of fixed values in one or more authorization fields would help to simplify authorization maintenance in roles.
      ...
      "! but all of whose authorization fields have to be maintained by the role administrator with customer-specific values.
      default_without_values TYPE ty_maintenance_status VALUE 'V',
      "! <p class="shorttext">Default Inactive</p>
      "! For objects with this status, the authorization check always returns the value sy-subrc = 0 regardless of which authorizations the user actually has.
      "! For objects belonging to the application areas Basis (S_*) and HR (P_* and PLOG), it is forbidden.
  ...
  "! Fields must have single or empty values. Empty fields trigger a DUMMY check.
  "! This object is auto-assigned in the transaction’s authorization defaults.
  TYPES ty_sao_afv_t TYPE STANDARD TABLE OF ty_sao_afv WITH EMPTY KEY.
  ...
  TYPES:
    ...
    "! Authorization default values define which authorization objects and field values are automatically proposed when creating roles for a transaction,
  ...
  TYPES:
    BEGIN OF ty_ad,
      ...
      "! Authorization defaults inheritance mode controls how authorization settings are passed from a parent transaction to its variants,
  ...
  TYPES:
    BEGIN OF ty_ua,
      ...
      "! Transaction UI Attributes Inheritance Mode controls how UI settings like screen variants or field attributes are
      "! inherited from a parent transaction to its variants, ensuring consistent user interfaces across related transactions.
      inheritance_mode TYPE abap_bool,
      "! <p class="shorttext">UI Classification</p>
      ...
      "! The Service Name of an Internet Application Component (IAC) links a transaction to a web service, enabling its execution via a browser.
      "! It's required for Easy Web Transactions if the service name differs from the transaction name.
      iac_service_name TYPE c LENGTH 14,
      "! <p class="shorttext">Pervasive Enabled</p>
      "! Transaction Pervasive Mode enables a simplified, responsive UI for transactions on various devices, like tablets or smartphones.
      "! It's designed for casual users needing quick, intuitive access to core functions with minimal screen complexity.
  ...
  TYPES:
    BEGIN OF ty_impl,
      ...
      "! It helps prevent unauthorized or unintended use, especially during maintenance, development, or when a transaction is obsolete or being phased out.
